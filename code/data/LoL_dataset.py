...
class LoL_Dataset_RIDCP(data.Dataset):
    def __init__(self, opt, train, all_opt):
        ...
        # self.pairs = [np.zeros((256, 256, 3)), np.zeros((256, 256, 3)), 'test']
    ...
    def load_pairs(self, folder_path):
        ...
        low_list = os.listdir(os.path.join(folder_path, 'dark'))
        print(len(low_list))
        #low_list = filter(lambda x: 'jpg' in x, low_list)
        pairs = []
        for idx, f_name in enumerate(low_list):
            pairs.append(
                [cv2.cvtColor(cv2.imread(os.path.join(folder_path, 'dark', f_name)), cv2.COLOR_BGR2RGB),  # [:, 4:-4, :],
                 cv2.imread(os.path.join(folder_path, 'normal', f_name)).astype(np.float32) / 255.0,
                 # [:, 4:-4, :],
                 f_name.split('.')[0]])
            # if idx > 10: break
