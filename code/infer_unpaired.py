...
def rgb(t):
    return (
        np.clip((t[0] if len(t.shape) == 4 else t).detach().cpu().numpy().transpose([1, 2, 0]), 0, 1) * 255
    ).astype(np.uint8)
...
def main():
    ...
    for lr_path, idx_test in tqdm.tqdm(zip(lr_paths, range(len(lr_paths)))):
        if os.path.basename(lr_path) in ['80.png', '89.png']:
            ...
            # 새로운 코드: 후처리 없이 모델의 출력값 그대로 저장
            sr = sr_t[0].detach().cpu().numpy().transpose([1, 2, 0])
            path_out_sr = os.path.join(test_dir, os.path.basename(lr_path))
            imwrite(path_out_sr, sr)
