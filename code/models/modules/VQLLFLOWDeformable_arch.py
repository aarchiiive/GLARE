...
class VQLLFLOWDeformable(nn.Module):
    ...
    # @autocast()
    def forward(self, net_vq=None, gt=None, lr=None, z=None, eps_std=None, reverse=True, epses=None, reverse_with_grad=False,
                lr_enc=None,
        ...
        if not reverse:
            ...
            if reverse_with_grad:
                ...
                with torch.no_grad():
                    print(f"add_gt_noise: {add_gt_noise}")
                    return self.reverse_flow(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
                                             add_gt_noise=add_gt_noise, net_vq=net_vq)
    ...
    def reverse_flow(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True, net_vq=None):
        ...
        # print(f"[pixels] Mean: {pixels.mean()}, Std: {pixels.std()}, Min: {pixels.min()}, Max: {pixels.max()}")
        ...
        if lr_enc is None and self.RRDB:
            with torch.no_grad():
                ...
                # print(f"[lr_enc] Mean: {lr_enc.mean()}, Std: {lr_enc.std()}, Min: {lr_enc.min()}, Max: {lr_enc.max()}")
        if self.opt['cond_encoder'] == "NoEncoder":
            z = lr[:,:3]
            # print(f"[z] Mean: {z.mean()}, Std: {z.std()}, Min: {z.min()}, Max: {z.max()}")
        else:
            if 'avg_color_map' in self.opt.keys() and self.opt['avg_color_map']:
        ...
        with torch.no_grad():
            x, logdet = self.flowUpsamplerNet(rrdbResults=lr_enc, z=z, eps_std=eps_std, reverse=True, epses=epses,
                                          logdet=logdet)
            # print(f"[x] Mean: {x.mean()}, Std: {x.std()}, Min: {x.min()}, Max: {x.max()}")
            # print(f"[logdet] Mean: {logdet.mean()}, Std: {logdet.std()}, Min: {logdet.min()}, Max: {logdet.max()}")
            enc_feat=x
            x= x.to('cuda:0')
