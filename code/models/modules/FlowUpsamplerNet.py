...
def safe_clip_tensor(tensor, min_val=-1e6, max_val=1e6):
    """
    float32 기준으로 underflow나 overflow를 방지하기 위해
    텐서 값을 주어진 범위로 클리핑하고, nan이나 inf가 있다면 대체합니다.
    """
...
class FlowUpsamplerNet(nn.Module):
    ...
    def forward(self, gt=None, rrdbResults=None, z=None, epses=None, logdet=0., reverse=False, eps_std=None,
        ...
        if reverse:                      #False
            ...
            # print(f'[FlowUpsamplerNet, rrdbResults] Mean: {rrdbResults.mean()}, Std: {rrdbResults.std()}, Min: {rrdbResults.min()}, Max: {rrdbResults.max()}')

            # for k, v in rrdbResults.items():
            #     if isinstance(v, torch.Tensor):
            #         print(f'[FlowUpsamplerNet, rrdbResults[{k}]] Mean: {v.mean()}, Std: {v.std()}, Min: {v.min()}, Max: {v.max()}')

            sr, logdet = self.decode(rrdbResults, z, eps_std, epses=epses_copy, logdet=logdet, y_onehot=y_onehot)
            # print(f'[FlowUpsamplerNet, sr] Mean: {sr.mean()}, Std: {sr.std()}, Min: {sr.min()}, Max: {sr.max()}')
            # print(f'[FlowUpsamplerNet, logdet] Mean: {logdet.mean()}, Std: {logdet.std()}, Min: {logdet.min()}, Max: {logdet.max()}')
            if self.sigmoid_output:
                sr = torch.sigmoid(sr)
                # print(f'[FlowUpsamplerNet, sigmoid(sr)] Mean: {sr.mean()}, Std: {sr.std()}, Min: {sr.min()}, Max: {sr.max()}')
            return sr, logdet
        else:
    ...
    #                 print(f"[level_conditionals[{level}]] Mean: {level_conditionals[level].mean()}, Std: {level_conditionals[level].std()}, Min: {level_conditionals[level].min()}, Max: {level_conditionals[level].max()}")
    ...
    #         print(f"[fl_fea] Mean: {fl_fea.mean()}, Std: {fl_fea.std()}, Min: {fl_fea.min()}, Max: {fl_fea.max()}")
    #         print(f"[logdet] Mean: {logdet.mean()}, Std: {logdet.std()}, Min: {logdet.min()}, Max: {logdet.max()}")
    #         print('='*100)
    #     sr = fl_fea
    ...
    def decode(self, rrdbResults, z, eps_std=None, epses=None, logdet=0.0, y_onehot=None):
        # epses가 리스트면 마지막 요소를 사용
        z = epses.pop() if isinstance(epses, list) else z

        # 입력 텐서를 그대로 사용 (float32 기준)
        fl_fea = z
        # logdet를 텐서로 변환 (이미 텐서인 경우에도 float32로)
        if not isinstance(logdet, torch.Tensor):
            logdet = torch.tensor(logdet, dtype=torch.float32, device=fl_fea.device)
        else:
            logdet = logdet.to(torch.float32)

        bypasses = {}
        level_conditionals = {}
        # rrdbResults에서 추출한 텐서들도 float32로 맞춰줍니다.
        if not opt_get(self.opt, ['network_G', 'flow', 'levelConditional', 'conditional']) == True:
            for level in range(5):
                if level not in self.levelToName.keys():
                    level_conditionals[level] = None
                else:
                    level_tensor = rrdbResults[self.levelToName[level]] if rrdbResults else None
                    if level_tensor is not None:
                        level_tensor = level_tensor.to(torch.float32)
                    level_conditionals[level] = level_tensor
                    # print(f"[level_conditionals[{level}]] Mean: {level_conditionals[level].mean()}, "
                    #     f"Std: {level_conditionals[level].std()}, Min: {level_conditionals[level].min()}, "
                    #     f"Max: {level_conditionals[level].max()}")

        # 디버깅을 위한 layers의 길이 출력
        # for _ in range(5):
        #     print(len(self.layers))

        # 각 레이어를 역순으로 통과시키면서 연산
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            size = shape[2]
            level = int(np.log(self.hr_size / size) / np.log(2))
            # print('=' * 100)
            ...
            if isinstance(layer, Split2d):
                fl_fea, logdet = self.forward_split2d_reverse(
                    eps_std, epses, fl_fea, layer,
                    rrdbResults[self.levelToName[level]].to(torch.float32),
                    logdet=logdet,
                    y_onehot=y_onehot)
            elif isinstance(layer, FlowStep):
                fl_fea = fl_fea.to(level_conditionals[level].device, dtype=torch.float32)
                fl_fea, logdet = layer(
                    fl_fea, logdet=logdet, reverse=True, rrdbResults=level_conditionals[level])
            else:
                fl_fea, logdet = layer(fl_fea, logdet=logdet, reverse=True)

            # 각 레이어 연산 후에 clipping을 적용하여 값의 범위를 제한합니다.
            # fl_fea = safe_clip_tensor(fl_fea)
            # logdet = safe_clip_tensor(logdet)
