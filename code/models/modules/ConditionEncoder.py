...
class ConEncoder1(nn.Module):
    ...
    def forward(self, x, mid_feat = False):
        ...
        # print(f"[cond_feat] Mean: {cond_feat.mean()}, Std: {cond_feat.std()}, Min: {cond_feat.min()}, Max: {cond_feat.max()}")
        color_feat = self.color_conv(enc_feat)
        # print(f"[color_feat] Mean: {color_feat.mean()}, Std: {color_feat.std()}, Min: {color_feat.min()}, Max: {color_feat.max()}")
