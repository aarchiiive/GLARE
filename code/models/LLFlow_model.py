...
class LLFlowModel(BaseModel):
    ...
    def forward(self, lr_left):
        sr_left = self.get_sr(lq=lr_left, heat=None)
        # lr_out = sr_left.to(sr_left.device)

        # lr_out, _, _ = self.net_hq.decode(lr_out)

        return sr_left

    # def get_flops(self, dummy_input):
    #     # netG에 대한 FLOP 계산
    #     flops_netG = FlopCountAnalysis(self.netG, dummy_input)
    #     print(f"FLOPs for netG: {flops_netG.total()}")

    #     # net_hq에 대한 FLOP 계산
    #     flops_net_hq = FlopCountAnalysis(self.net_hq, dummy_input)
    #     print(f"FLOPs for net_hq: {flops_net_hq.total()}")

    #     total_flops = flops_netG.total() + flops_net_hq.total()
